...
const findChild = (instance, checkFn) => {
  ...
  for (let i = 0, l = $children(instance).length; i < l; i++) {
    const child = $children(instance)[i]
...
const findOptionsInVNode = (node) => {
  ...
  if (opts && opts.Ctor.name.match(optionRegexp)) return [node]
...
export default {
  ...
  computed: {
    ...
    selectOptions() {
      ...
      for (let option of slotOptions) {
        if (option.componentOptions && option.type.name.match(optionGroupRegexp)) {
          const cOptions = option.componentOptions
          let children = cOptions.children.default()[0].children
  ...
  methods: {
    ...
    toggleMenu(e, force) {
      ...
      if (this.visible) {
        ...
        $emit(this.$refs.dropdown, 'on-update-popper')
      }
    },
    hideMenu() {
    ...
    updateSlotOptions() {
      this.slotOptions = this.$slots.default && this.$slots.default()[0].children
    },
    checkUpdateStatus() {
      if (
  ...
  watch: {
    ...
    values(now, before) {
      ...
      if (shouldEmitInput) {
        $emit(this, 'update:value', vModelValue) // to update v-model
        $emit(this, 'on-change', this.publicValue)
        this.dispatch('FormItem', 'on-form-change', this.publicValue)
      }
    },
    query(query) {
